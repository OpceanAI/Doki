...
\begin{itemize}[leftmargin=*, itemsep=4pt]
  ...
  \item \textbf{NAT traversal.} Implement STUN/TURN hole-punching
        for peer discovery across NAT boundaries. This enables
        DokiLink connectivity between devices on different networks
        without static IP addresses.

  \item \textbf{DHT peer discovery.} Distributed hash table for
        decentralized peer discovery without static configuration.
        This eliminates the need for manually adding peers via
        \texttt{doki link add}.

  \item \textbf{Kubernetes compatibility.} Full CRI (Container
        Runtime Interface) compliance for Kubernetes node integration.
        This would enable Doki to serve as a runtime for Kubernetes
        pods on Android devices.
\end{itemize}
